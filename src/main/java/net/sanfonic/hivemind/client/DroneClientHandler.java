...
public class DroneClientHandler implements ClientModInitializer {
  ...
  private static final float INPUT_THRESHOLD =
      0.001f; // Lower threshold makes camera response feel less sticky
  ...
  private static void registerNetworkHandlers() {
    ...
    ClientPlayNetworking.registerGlobalReceiver(
        NetworkHandler.DRONE_CONTROL_PACKET,
  ...
  public static void handleDroneControl(MinecraftClient client) {
    ...
    // Use that as the source of truth, then mirror it onto the drone locally for smooth camera
    // motion.
    ...
    boolean rotationChanged =
        Math.abs(currentYaw - lastYaw) > INPUT_THRESHOLD
            || Math.abs(currentPitch - lastPitch) > INPUT_THRESHOLD;

    boolean movementChanged =
        Math.abs(forward - lastForward) > INPUT_THRESHOLD
    ...
    boolean shouldSendPacket =
        rotationChanged || movementChanged || hasActiveInput || (packetTimer >= PACKET_SEND_RATE);

    // Send movement packet to server
    if (shouldSendPacket) {
      DroneMovementPacket packet =
          new DroneMovementPacket(forward, strafe, up, jumping, sneaking, currentYaw, currentPitch);
      ...
      if (player.age % 60 == 0 && (hasActiveInput || rotationChanged)) {
        Hivemind.LOGGER.debug(
            "Drone input movement F={} S={} U={} rotation Y={} P={}",
  ...
  public static void init() {
    ...
    Hivemind.LOGGER.debug(
        "DroneClientHandler.init() called; initialization happens in onInitializeClient()");
  }
}
