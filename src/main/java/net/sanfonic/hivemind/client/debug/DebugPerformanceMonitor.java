...
public class DebugPerformanceMonitor {
  ...
  public static void tick(MinecraftClient client) {
    ...
    if (lastUpdateTime > 0) {
      ...
      // Calculate average
      avgFrameTime =
          frameTimeSamples.stream().mapToLong(Long::longValue).average().orElse(0.0)
    ...
    if (client.world != null) {
      ...
      avgDroneCount = droneCountSamples.stream().mapToInt(Integer::intValue).average().orElse(0.0);
    }
  }
  ...
  private static void renderPerformanceHud(DrawContext context, float tickDelta) {
    ...
    lines.add(
        Text.literal("=== HiveMind Performance ===").formatted(Formatting.GOLD, Formatting.BOLD));
    lines.add(
        Text.literal(String.format("FPS: %.1f (%.2fms)", fps, avgFrameTime))
            .formatted(getFpsColor(fps)));
    lines.add(Text.literal(""));
    lines.add(Text.literal("Drones:").formatted(Formatting.YELLOW));
    lines.add(
        Text.literal(String.format("  Total: %d (avg: %.1f)", totalDrones, avgDroneCount))
            .formatted(Formatting.WHITE));
    lines.add(
        Text.literal(String.format("  Linked: %d", linkedDrones)).formatted(Formatting.GREEN));
    lines.add(
        Text.literal(String.format("  Controlled: %d", controlledDrones))
    ...
    lines.add(
        Text.literal(String.format("  Used: %dMB / %dMB", usedMemory, maxMemory))
    ...
    int textY = y + padding;
    for (Text line : lines) {
      context.drawTextWithShadow(client.textRenderer, line, x + padding, textY, 0xFFFFFF);
      textY += lineHeight;
    }
  }
