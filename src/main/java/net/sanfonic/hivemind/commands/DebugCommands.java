...
public class DebugCommands {
    ...
    hiveCommand
        .then(
            CommandManager.literal("debug")
                ...
                .then(CommandManager.literal("toggle").executes(DebugCommands::toggleDebugMode))
                .then(
                    CommandManager.literal("spawn")
                        .executes(DebugCommands::spawnDebugDrone)
                        .then(
                            CommandManager.argument("count", IntegerArgumentType.integer(1, 10))
                ...
                .then(
                    CommandManager.literal("setrole")
                        .then(
                            CommandManager.argument("drone", EntityArgumentType.entity())
                                .then(
                ...
                .then(
                    CommandManager.literal("killall")
                        ...
                        .then(
                            CommandManager.argument("radius", IntegerArgumentType.integer(1, 500))
                                .executes(DebugCommands::killAllDronesInRadius)))
                .then(CommandManager.literal("grantaccess").executes(DebugCommands::grantAccess))
                .then(CommandManager.literal("status").executes(DebugCommands::showDebugStatus))
                .then(
                    CommandManager.literal("teleport")
                        .then(
                            CommandManager.literal("tome")
                                .then(
                                    CommandManager.argument("drone", EntityArgumentType.entity())
                                        .executes(DebugCommands::teleportDroneToPlayer)))
                        .then(
                            CommandManager.literal("todrone")
                                .then(
                                    CommandManager.argument("drone", EntityArgumentType.entity())
                            ...
                            CommandManager.argument("drone", EntityArgumentType.entity())
                                .then(
                                    CommandManager.argument(
                                            "amount", IntegerArgumentType.integer(1, 100))
                                        .executes(DebugCommands::damageDrone)))))
        .then(CommandManager.literal("dump").executes(DebugCommands::dumpHiveState))
        .then(
  ...
  private static int spawnDrones(CommandContext<ServerCommandSource> context, int count) {
    try {
      ...
      for (int i = 0; i < count; i++) {
        ...
        BlockPos spawnPos =
            new BlockPos(
                (int) (playerPos.x + offsetX), (int) playerPos.y, (int) (playerPos.z + offsetZ));
      ...
      context
          ...
          .sendFeedback(
              () ->
                  Text.literal("✓ Spawned ")
                      .formatted(Formatting.GREEN)
                      .append(
                          Text.literal(String.valueOf(finalSpawned))
  ...
  private static int showDroneInfo(CommandContext<ServerCommandSource> context) {
    try {
      ...
      context
          ...
          .sendFeedback(
              () ->
                  Text.literal("HiveCode: ")
                      .formatted(Formatting.GRAY)
                      .append(Text.literal(hiveCode).formatted(Formatting.AQUA)),
              false);
          ...
          .sendFeedback(
              () ->
                  Text.literal("UUID: ")
                      .formatted(Formatting.GRAY)
                      .append(
                          Text.literal(drone.getUuid().toString()).formatted(Formatting.DARK_GRAY)),
              false);
              ...
              () ->
                  Text.literal("Health: ")
                      ...
                      .append(
                          Text.literal(
                                  String.format(
                                      "%.1f/%.1f", drone.getHealth(), drone.getMaxHealth()))
          ...
          .sendFeedback(
              () ->
                  Text.literal("Role: ")
                      .formatted(Formatting.GRAY)
                      .append(
                          Text.literal(drone.getRole().getDisplayName())
              ...
              () ->
                  Text.literal("Position: ")
                      ...
                      .append(
                          Text.literal(
                                  String.format(
                                      "%.1f, %.1f, %.1f", drone.getX(), drone.getY(), drone.getZ()))
      ...
      if (drone.hasHiveMindOwner()) {
        ...
        context
            ...
            .sendFeedback(
                () ->
                    Text.literal("Owner UUID: ")
                        .formatted(Formatting.GRAY)
                        .append(Text.literal(ownerUuid.toString()).formatted(Formatting.GREEN)),
                false);
      } else {
        context
            .getSource()
      ...
      context
          ...
          .sendFeedback(
              () ->
                  Text.literal("AI Paused: ")
                      .formatted(Formatting.GRAY)
                      .append(
                          Text.literal(String.valueOf(drone.isAiControlPaused()))
                              .formatted(
                                  drone.isAiControlPaused() ? Formatting.RED : Formatting.GREEN)),
              false);
  ...
  private static int setDroneRole(CommandContext<ServerCommandSource> context) {
    try {
      ...
      serverWorld
          .getChunkManager()
          .sendToNearbyPlayers(
              drone, drone.createSpawnPacket()); // This forces a full entity update

      context
          .getSource()
          .sendFeedback(
              () ->
                  Text.literal("✓ Changed drone role from ")
                      .formatted(Formatting.GREEN)
                      .append(Text.literal(oldRole.getDisplayName()).formatted(Formatting.YELLOW))
                      .append(Text.literal(" to ").formatted(Formatting.GREEN))
                      .append(
                          Text.literal(newRole.getDisplayName())
                              .formatted(Formatting.GOLD, Formatting.BOLD)),
              false);
  ...
  private static int killAllDronesInRadius(
      CommandContext<ServerCommandSource> context, int radius) {
    try {
      ServerPlayerEntity player = context.getSource().getPlayerOrThrow();
      ServerWorld world = player.getServerWorld();
      ...
      List<DroneEntity> drones =
          world.getEntitiesByClass(
              DroneEntity.class, player.getBoundingBox().expand(radius), drone -> true);
          ...
          .sendFeedback(
              () ->
                  Text.literal("✗ Removed ")
                      .formatted(Formatting.RED)
                      .append(
                          Text.literal(String.valueOf(finalKilled))
                              .formatted(Formatting.GOLD, Formatting.BOLD))
    ...
    try {
      ...
      if (PlayerHiveComponent.hasAccess(player)) {
        context
            ...
            .sendFeedback(
                () ->
                    Text.literal("⚠ You already have HiveMind access!")
                        .formatted(Formatting.YELLOW),
                false);
        return 0;
      }
  ...
  private static int showDebugStatus(CommandContext<ServerCommandSource> context) {
    ...
    context
        ...
        .sendFeedback(
            () ->
                Text.literal("Debug Mode: ")
                    .append(
                        Text.literal(config.isDebugModeEnabled() ? "ENABLED" : "DISABLED")
                            .formatted(
                                config.isDebugModeEnabled() ? Formatting.GREEN : Formatting.RED)),
            false);
    ...
    if (config.isDebugModeEnabled()) {
      ...
      context
          .getSource()
          .sendFeedback(
              () -> Text.literal("  Auto-Link: " + config.debugAutoLink).formatted(Formatting.GRAY),
              false);
      context
          .getSource()
          .sendFeedback(
  ...
  private static int teleportDroneToPlayer(CommandContext<ServerCommandSource> context) {
    try {
      ...
      context
          ...
          .sendFeedback(
              () ->
                  Text.literal("§bTeleported drone ")
  ...
  private static int teleportPlayerToDrone(CommandContext<ServerCommandSource> context) {
    try {
      ...
      world.spawnParticles(
          ParticleTypes.PORTAL, dronePos.x, dronePos.y + 1, dronePos.z, 30, 0.5, 0.5, 0.5, 0.5);
      ...
      context
          ...
          .sendFeedback(
              () ->
                  Text.literal("§bTeleported to drone ")
                      .formatted(Formatting.AQUA)
                      .append(Text.literal(hiveCode).formatted(Formatting.GOLD)),
              false);
  ...
  private static int teleportNearestDrone(CommandContext<ServerCommandSource> context) {
    try {
      ...
      DroneEntity nearestDrone =
          world
              .getEntitiesByClass(
                  DroneEntity.class, player.getBoundingBox().expand(100), drone -> true)
              .stream()
      ...
      context
          ...
          .sendFeedback(
              () ->
                  Text.literal("§bTeleported nearest drone ")
                      .formatted(Formatting.AQUA)
                      .append(Text.literal(hiveCode).formatted(Formatting.GOLD))
                      .append(Text.literal(" to you").formatted(Formatting.AQUA)),
              false);
  ...
  private static void teleportDroneWithSync(DroneEntity drone, Vec3d targetPos, ServerWorld world) {
    ...
    world.getChunkManager().sendToNearbyPlayers(drone, new EntityPositionS2CPacket(drone));

    // Particles at old position
    world.spawnParticles(
        ParticleTypes.PORTAL, oldPos.x, oldPos.y + 1, oldPos.z, 30, 0.5, 0.5, 0.5, 0.5);

    // Particles at new position
    world.spawnParticles(
        ParticleTypes.PORTAL, targetPos.x, targetPos.y + 1, targetPos.z, 30, 0.5, 0.5, 0.5, 0.5);
          ...
          .sendFeedback(
              () ->
                  Text.literal("§cDealt ")
                      .formatted(Formatting.RED)
                      .append(
                          Text.literal(String.format("%.1f", amount)).formatted(Formatting.GOLD))
  ...
  private static int dumpHiveState(CommandContext<ServerCommandSource> context) {
    try {
      ServerCommandSource source = context.getSource();
      MinecraftServer server = source.getServer();

      source.sendFeedback(
          () -> Text.literal("=== HiveMind State Dump ===").formatted(Formatting.GOLD), false);

      // Get all drones for diagnostics
      int[] droneCount = {0};
      for (ServerWorld world : server.getWorlds()) {
        for (Entity entity : (Iterable<Entity>) world.iterateEntities()) {
          if (entity instanceof DroneEntity drone) {
            droneCount[0]++;
            source.sendFeedback(
                () ->
                    Text.literal("  Drone ")
                        .formatted(Formatting.GRAY)
                        .append(Text.literal(drone.getHiveCode()).formatted(Formatting.AQUA))
      ...
      final int count = droneCount[0];
      source.sendFeedback(
          () ->
              Text.literal("Total drones: ")
                  .formatted(Formatting.GRAY)
                  .append(Text.literal(String.valueOf(count)).formatted(Formatting.GOLD)),
          false);

      source.sendFeedback(() -> Text.literal("Dump complete!").formatted(Formatting.GREEN), false);
    ...
    try {
      ...
      if (drone.hasHiveMindOwner()) {
        ...
        context
            ...
            .sendFeedback(
                () ->
                    Text.literal("Owner UUID: " + ownerUuid.toString()).formatted(Formatting.GREEN),
                false);
      } else {
        context
            .getSource()
            .sendFeedback(() -> Text.literal("Owner: None").formatted(Formatting.RED), false);
      }
  ...
  public static void addHiveCommands(LiteralArgumentBuilder<ServerCommandSource> hiveCommand) {
    hiveCommand
        .then(CommandManager.literal("join").executes(DebugCommands::joinMyHive))
        .then(CommandManager.literal("leave").executes(DebugCommands::leaveHive));
  }

  private static int joinMyHive(CommandContext<ServerCommandSource> context) {
    try {
      ServerPlayerEntity player = context.getSource().getPlayerOrThrow();
      ServerCommandSource source = context.getSource();
      MinecraftServer server = source.getServer();

      net.sanfonic.hivemind.data.HiveMindData.HiveManager hiveManager =
          net.sanfonic.hivemind.data.HiveMindData.HiveManager.getInstance(server);
      net.sanfonic.hivemind.data.HiveMindData.Hive hive =
          hiveManager.getOrCreateHive(player.getUuid());

      source.sendFeedback(
          () ->
              Text.literal("✓ ")
                  .formatted(Formatting.GREEN)
                  .append(Text.literal("You have a hive!").formatted(Formatting.GRAY))
                  .append(Text.literal(" ").formatted(Formatting.GRAY))
                  .append(
                      Text.literal(hive.getId().toString().substring(0, 8))
                          .formatted(Formatting.GOLD)),
          false);
  ...
  private static int leaveHive(CommandContext<ServerCommandSource> context) {
    try {
      ServerPlayerEntity player = context.getSource().getPlayerOrThrow();
      ServerCommandSource source = context.getSource();
      MinecraftServer server = source.getServer();
      ...
      net.sanfonic.hivemind.data.HiveMindData.HiveManager hiveManager =
          net.sanfonic.hivemind.data.HiveMindData.HiveManager.getInstance(server);
      net.sanfonic.hivemind.data.HiveMindData.Hive ownerHive =
          hiveManager.getHiveByOwner(player.getUuid());

      if (ownerHive != null) {
        source.sendError(Text.literal("You cannot leave your own hive! You must delete it."));
        return 0;
      }

      // Find hive this player is a member of (but doesn't own)
      for (net.sanfonic.hivemind.data.HiveMindData.Hive hive : hiveManager.getAllHives().values()) {
        if (hive.isMember(player.getUuid()) && !hive.getOwnerUuid().equals(player.getUuid())) {
          hiveManager.removeMemberFromHive(hive.getId(), player.getUuid());
          source.sendFeedback(
              () ->
                  Text.literal("✓ ")
                      .formatted(Formatting.GREEN)
                      .append(Text.literal("You have left the hive.").formatted(Formatting.GRAY)),
              false);
          return 1;
        }
      }

      source.sendError(Text.literal("You are not a member of any hive!"));
      return 0;
