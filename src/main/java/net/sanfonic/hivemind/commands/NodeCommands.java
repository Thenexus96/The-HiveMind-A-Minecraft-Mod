...
public class NodeCommands {
    public static void addNodeCommands(LiteralArgumentBuilder<ServerCommandSource> hiveCommand) {
        hiveCommand.then(CommandManager.literal("node")
    ...
    private static int claimNode(CommandContext<ServerCommandSource> context) {
        try {
            ...
            if (!ok) {
                source.sendError(Text.literal("Failed to claim node. Ensure you have a hive and node exists."));
                return 0;
            }

            source.sendFeedback(() -> Text.literal("Node claimed: " + nodeId).formatted(Formatting.GREEN), false);
            return 1;
        } catch (Exception e) {
