...
public class HiveMindConfigCommand {
        ...
        ((commandDispatcher, commandRegistryAccess, registrationEnvironment) -> {
          commandDispatcher.register(
              CommandManager.literal("hivemind_config")
                  ...
                  .then(CommandManager.literal("save").executes(HiveMindConfigCommand::saveConfig))
                  .then(CommandManager.literal("show").executes(HiveMindConfigCommand::showConfig))
                  .then(
                  ...
                  .then(
                      CommandManager.literal("drone_debug")
                          .then(
                              CommandManager.argument("enabled", BoolArgumentType.bool())
                                  .executes(HiveMindConfigCommand::setDroneLinkingDebug)))
                  .then(
                      CommandManager.literal("command_debug")
                          .then(
                              CommandManager.argument("enabled", BoolArgumentType.bool())
                  ...
                  .then(
                      CommandManager.literal("drone_health")
                          .then(
                              CommandManager.argument(
                                      "health", DoubleArgumentType.doubleArg(1.0, 100.0))
                                  .executes(HiveMindConfigCommand::setDroneHealth)))
                  .then(
                      CommandManager.literal("drone_speed")
                          .then(
                              CommandManager.argument(
                                      "speed", DoubleArgumentType.doubleArg(0.1, 2.0))
                                  .executes(HiveMindConfigCommand::setDroneSpeed)))
                  .then(
                      CommandManager.literal("debug_mode")
                          .then(
                              CommandManager.argument("enabled", BoolArgumentType.bool())
    ...
    context
        ...
        .sendFeedback(
            () ->
                Text.literal(
                    "§7Drone Link Debug: §"
                        + (config.enableDroneLinkingDebug ? "a" : "c")
                        + config.enableDroneLinkingDebug),
            false);
    context
        .getSource()
        .sendFeedback(
            () ->
                Text.literal(
                    "§7Command Debug: §"
                        + (config.enableCommandDebug ? "a" : "c")
                        + config.enableCommandDebug),
            false);
    context
        .getSource()
        .sendFeedback(
            () -> Text.literal("§7Log Cooldown: §b" + config.logCooldownSeconds + "s"), false);
    context
        .getSource()
        .sendFeedback(() -> Text.literal("§7Drone Health: §b" + config.droneHealth), false);
    context
        .getSource()
        .sendFeedback(() -> Text.literal("§7Drone Speed: §b" + config.droneSpeed), false);
    context
        .getSource()
        .sendFeedback(
            () -> Text.literal("§7Drone Follow Range: §b" + config.droneFollowRange), false);
    context
        .getSource()
        .sendFeedback(
            () ->
  ...
  private static int setDebugLogging(CommandContext<ServerCommandSource> context) {
    ...
    context
        .getSource()
        .sendFeedback(
            () -> Text.literal("§aDebug logging " + (enabled ? "enabled" : "disabled")), false);
    return 1;
  }
  ...
  private static int setDroneLinkingDebug(CommandContext<ServerCommandSource> context) {
    ...
    context
        .getSource()
        .sendFeedback(
            () -> Text.literal("§aDrone linking debug " + (enabled ? "enabled" : "disabled")),
            false);
    return 1;
  }
  ...
  private static int setCommandDebug(CommandContext<ServerCommandSource> context) {
    ...
    context
        .getSource()
        .sendFeedback(
            () -> Text.literal("§aCommand debug " + (enabled ? "enabled" : "disabled")), false);
    return 1;
  }
  ...
  private static int setLogCooldown(CommandContext<ServerCommandSource> context) {
    ...
    context
        .getSource()
        .sendFeedback(() -> Text.literal("§aLog cooldown set to " + seconds + " seconds"), false);
    return 1;
  }
  ...
  private static int setDroneHealth(CommandContext<ServerCommandSource> context) {
    ...
    context
        ...
        .sendFeedback(
            () ->
                Text.literal(
                    "§aDrone health set to " + health + " (requires restart for existing drones)"),
            false);
    return 1;
  }
  ...
  private static int setDroneSpeed(CommandContext<ServerCommandSource> context) {
    ...
    context
        ...
        .sendFeedback(
            () ->
                Text.literal(
                    "§aDrone speed set to " + speed + " (requires restart for existing drones)"),
            false);
    return 1;
  }
  ...
  private static int setDebugMode(CommandContext<ServerCommandSource> context) {
    ...
    context
        .getSource()
        .sendFeedback(
            () -> Text.literal("§aDebug mode " + (enabled ? "enabled" : "disabled")), false);
    return 1;
  }
}
