...
public class DroneControlManager {
  ...
  public static void init() {
    ...
    ServerTickEvents.END_SERVER_TICK.register(
        server -> {
          ...
          List<ServerPlayerEntity> playersToRelease =
  ...
  private static void savePlayerStates(ServerPlayerEntity player) {
    ...
    PlayerState state =
        new PlayerState(player.getPos(), player.getPitch(), player.getYaw(), copiedAbilites);
  ...
  public static boolean isDroneControlled(DroneEntity drone) {
    return activeSessions.values().stream().anyMatch(session -> session.getDrone().equals(drone));
  }
