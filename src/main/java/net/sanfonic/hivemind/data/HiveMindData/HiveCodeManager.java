...
/**
 * Manages HiveCode assignment for drones with PER-PLAYER counters Each player has their own drone
 * numbering D-001, D-002, D-003, etc. Numbers are never reused - if D-002 dies, the next drone is
 * D-004
 */
public class HiveCodeManager extends PersistentState {
  ...
  public static HiveCodeManager getInstance(MinecraftServer server) {
    ...
    return stateManager.getOrCreate(
        HiveCodeManager::createFromNbt, HiveCodeManager::new, DATA_NAME);
  }
  ...
  /**
   * Generate a new HiveCode for a drone Uses the player's personal counter
   *
  ...
  public String generateHiveCode(UUID droneUUID, UUID ownerUUID) {
    ...
    Hivemind.LOGGER.debug(
        "Generated HiveCode {} for player {} (player's drone #{})",
        hiveCode,
  ...
  /**
   * Get the drone from UUID from a HiveCode Note: Multiple players can have D-001, so you need to
   * filter by owner
   *
  ...
  /**
   * Remove a drone's HiveCode (called when drone dies or is removed) Note: The player's counter is
   * NOT decremented - numbers are never reused
   *
   * @param droneUUID The drone's UUID
   */
