...
public class DroneEntity extends PathAwareEntity {
  ...
  /** Compatibility alias for older callers. Use {@link #setHiveMindOwner(UUID)} for new code. */
  public void setHiveMindOwnerUuid(UUID ownerUUID) {
    setHiveMindOwner(ownerUUID);
  }
  ...
  public void pauseAIControl() {
    if (!aiControlPaused) {
      ...
      this.savedTargetGoals =
          new GoalSelector(() -> this.getWorld().getProfiler()); // Save current goals
  ...
  public void triggerSecondaryAbility() {
    ...
    // this.getWorld().getEntitiesByClass(LivingEntity.class, this.getBoundingBox().expand(5.0),
    // null)
    //     .forEach(entity -> entity.heal(2.0F));
  }

  // Method to get the actual player object (if they're online, Alternative method that takes a
  // world parameter
  ...
  /**
   * Restores the HiveMind Connection from persistent data Called when the server loads and needs to
   * restore drone-owner relationships
   */
  public void restoreHiveMindConnection() {
    if (this.getWorld().isClient) return; // Only run on server

    log.debug("Attempting to restore connection for drone: {}", this.getUuid());

    MinecraftServer server = this.getWorld().getServer();
    if (server == null) {
      log.warn("Server is null, cannot restore drone connection");
      return;
    }

    // Use the DroneLinkUtils helper to get owner mappings
    UUID ownerUUID =
        net.sanfonic.hivemind.service.DroneLinkUtils.getDroneOwner(server, this.getUuid());
    if (ownerUUID == null) {
    ...
    if (ownerUUID != null) {
      ...
      if (owner != null) {
        ...
        if (ownerName != null) {
          ...
          log.debug("Set owner name from cache: {}", ownerName);
        } else {
          // Fallback to "Unknown Owner"
          this.dataTracker.set(IS_LINKED, true);
          this.dataTracker.set(OWNER_NAME, "Unknown Owner");
          log.debug("Could not find owner name, using Unknown Owner");
        }

        // Owner is offline, but we can still set the tracked data
        this.dataTracker.set(IS_LINKED, true);
      }
      log.debug("Successfully restored drone connection");
    } else {
      log.debug("No owner found for drone");
    }
  }

  /** Try to get player's name from thier UUID Works even if the player is offline */
  private String getPlayerNameFromUUID(MinecraftServer server, UUID playerUUID) {
    try {
      ...
      com.mojang.authlib.GameProfile profile =
          server.getUserCache().getByUuid(playerUUID).orElse(null);
      ...
      log.warn("Error getting player name", e);
      return null;
    }
  }
  ...
  private void updateTrackedData(PlayerEntity owner) {
    if (owner != null) {
      ...
      log.debug("Updated tracked data for owner: {}", owner.getName().getString());
      ...
      if (!this.getWorld().isClient && this.getWorld().getServer() != null) {
        String ownerName =
            getPlayerNameFromUUID(this.getWorld().getServer(), this.hiveMindOwnerUuid);
        if (ownerName != null) {
          this.dataTracker.set(OWNER_NAME, ownerName);
          log.debug("Updated tracked data for offline owner");
        }
      } else {
        this.dataTracker.set(IS_LINKED, false);
        this.dataTracker.set(OWNER_NAME, "");
        log.debug("Cleared tracked data");
      }
    }
  }
  ...
  /**
   * Apply the current role's behavior to this drone This clears role specific goals and re-applies
   * them
   */
    ...
    if (!this.getWorld().isClient) {
      ...
      if (server != null) {
        String generatedCode =
            net.sanfonic.hivemind.service.DroneLinkUtils.linkDrone(
                server,
                this.getUuid(),
                ownerUUID,
                this.getX(),
        ...
        if (generatedCode != null) {
          ...
          ModConfig.getInstance()
              .droneLinkDebugLog("Assigned HiveCode " + this.hiveCode + " to drone");
        }
      }
    ...
    if (this.getWorld().isClient) {
      ...
      if (data.equals(DRONE_YAW) || data.equals(DRONE_PITCH)) {
        try {
          ...
          if (DroneClientHandler.isControllingDrone()
  ...
  private boolean isLocallyControlled() {
    ...
    try {
      ...
      isLocallyControlled =
          net.sanfonic.hivemind.client.DroneClientHandler.isControllingDrone()
  ...
  public ActionResult interactMob(PlayerEntity player, Hand hand) {
    if (!this.getWorld().isClient) {
      ...
      String codeInfo =
          (hiveCode != null && !hiveCode.isEmpty()) ? "Code: " + hiveCode + " | " : "";

      Text message =
          Text.literal(
                  codeInfo + "Health: " + (int) this.getHealth() + "/" + (int) this.getMaxHealth())
              .formatted(Formatting.AQUA)
              .append(
                  Text.literal(" | Role: " + this.currentRole.getDisplayName())
                      .formatted(Formatting.YELLOW))
              .append(
                  Text.literal(" | " + ownerInfo)
                      .formatted(hasHiveMindOwner() ? Formatting.GREEN : Formatting.GRAY));
  ...
  public void readCustomDataFromNbt(NbtCompound nbt) {
    ...
    log.debug("DroneEntity reading from NBT");

    if (nbt.containsUuid("HiveMindOwner")) {
      UUID loadedUuid = nbt.getUuid("HiveMindOwner");
      if (isValidUUID(loadedUuid)) {
        this.hiveMindOwnerUuid = loadedUuid;
        log.debug("Loaded owner UUID from NBT: {}", loadedUuid);
    ...
    if (nbt.contains("HiveCode")) {
      ...
      this.dataTracker.set(HIVE_CODE, this.hiveCode);
      log.debug("Loaded HiveCode from NBT: {}", this.hiveCode);
    }
  }
  ...
  private void spawnLinkParticles() {
    ...
    for (int i = 0; i < 3; i++) {
      double angle =
          (System.currentTimeMillis() / 1000.0 + i + 2.0943951)
              % (Math.PI * 2); // 120 degrees apart
      double radius = 0.8;
