...
/**
 * Small helper utilities to perform common link/telemetry operations. Extracted from DroneEntity
 * logic so it can be unit-tested without constructing entities.
 */
public final class DroneLinkUtils {
  private DroneLinkUtils() {}

  /**
   * Link a drone to an owner and update telemetry + generate a hive code. Returns the generated
   * hive code (or null if none generated).
   */
  public static String linkDrone(
  ...
  // Test-friendly counterparts that accept an explicit HiveMindService to avoid mocking
  // MinecraftServer
  public static String linkDroneWithService(
      HiveMindService service,
      UUID droneUuid,
      UUID ownerUuid,
      double x,
      double y,
  ...
  public static UUID getDroneOwner(MinecraftServer server, UUID droneUuid) {
    if (server == null) return null;
    return getDroneOwnerWithService(HiveMindServiceManager.get(), droneUuid);
  }
}
